...
14. ✅ **เทียบ benchmark ต่อเนื่อง (E)** (เสร็จ 2026-07-17 · แก้ฐานการเทียบ 2026-08-08 — `portfolio/benchmark.py`: `shadow_benchmark` เอา **กระแสเงินเข้า-ออกจากภายนอกชุดเดียวกัน** ไปลง VOO (ไม้ซื้อ = เงินเข้า, ปันผลที่รับ = เงาต้อง**ขาย**เท่ากันวันเดียวกัน — จำเป็นเพราะราคาที่ใช้เป็น Adjusted Close = total return ขณะที่พอร์ตจริงตีด้วยราคาล้วน วัดจริงเอียงเข้าข้าง VOO 1.32 จุด/ปี) + `xirr` %/ปี money-weighted จากไม้ชุดเดียวกับมูลค่าปลายทาง; หน้า Portfolio โชว์เทียบ + real return หลังเงินเฟ้อไทยเมื่อรู้ CPI; พอร์ตอายุ < 90 วัน = ไม่ตีเป็น %/ปี)
