...
ตรวจ `emergency_fund_service` · `debt_service` · `networth_service` (บางส่วน) — ส่วนที่เหลือ
(`cashflow_service` / `report_service`) อยู่ในรอบที่ 2 ข้างล่าง
...
## ผลตรวจโมดูลที่ยังไม่เคยตรวจ — รอบที่ 2 (2026-09-01) · ครบทั้ง 5 โมดูลแล้ว

ตรวจ `cashflow_service` (348 บรรทัด) และ `report_service` (580 บรรทัด) ที่เหลือ
**พบบั๊กจริงอีก 2 ข้อ ทั้งคู่เป็นชนิด "ตัวเลขหน้าตาปกติที่อ่านผิด" ไม่มีอะไรพัง**

### ① `report_service.get_networth_change()` — เปอร์เซ็นต์พลิกเครื่องหมายเมื่อฐานติดลบ 🔴

`change_thb / previous_nw` ใช้ได้ก็ต่อเมื่อฐานเป็นบวก แต่ **มูลค่าสุทธิติดลบได้จริง**
(หนี้บ้าน/หนี้เรียนมากกว่าทรัพย์สิน = สถานะปกติของคนเริ่มต้น) วัดจริง 2026-09-01:

| ก่อน | ตอนนี้ | เปลี่ยนจริง | ที่รายงาน |
|---|---|---|---|
| −1,000 | −500 | **ดีขึ้น +500 บาท** | **−50%** (อ่านว่าแย่ลง) |
| −1,000 | −1,500 | **แย่ลง −500 บาท** | **+50%** (อ่านว่าดีขึ้น) |

อันตรายกว่าการหารด้วยศูนย์ตรงที่ **ได้ตัวเลขที่หน้าตาใช้ได้** ไม่มี exception ไม่มีอะไร
พัง และตัวเลขนี้ไหลเข้าทั้งรายงานที่ส่ง Telegram อัตโนมัติทุกวันที่ 1 และพรอมป์ที่ให้
LLM อธิบาย ⇒ AI เขียนบรรยายทิศทางกลับด้านจากความจริง

แก้เป็น `None` + เหตุผลไทย ไม่ใช่หารด้วย `abs(previous_nw)` เพราะ "ดีขึ้น 50% ของหนี้"
ไม่ใช่ประโยคที่มีความหมายทางการเงิน — **ยอดบาทยังถูกเสมอและยังแสดงต่อ** สิ่งที่ตัดออก
คือเปอร์เซ็นต์เท่านั้น · `_networth_txt` เดิมฮาร์ดโค้ดเหตุผลว่า "(ฐานเป็น 0)" เสมอ
จึงต้องยกเหตุผลจริงจากตัวคำนวณมาแทน (มีสองสาเหตุ ไม่ใช่หนึ่ง)

### ② `cashflow_service.detect_category_anomalies()` — ตัวเลขไม่มีหน้าต่างกำกับ 🟠

ฟังก์ชันเทียบ "เดือนที่จบแล้วและ**มีธุรกรรม**ล่าสุด" กับค่าเฉลี่ยของเดือนก่อน ๆ —
เดือนนั้น **ไม่ใช่ "เดือนที่แล้ว" เสมอไป** วัดจริง: ธุรกรรมล่าสุด 2026-03 วันนี้ 2026-09
API ตอบ `"ค่าอาหาร +76.5%"` โดยไม่มีช่องไหนบอกว่าเป็นเดือนไหนหรือเฉลี่ยจากกี่เดือน
(`CategoryAnomaly.last_month` เป็น **จำนวนเงิน** ไม่ใช่เดือน) หน้าจอจึงแสดงเป็นข่าวของ
เดือนนี้ได้เต็มปาก — ความผิดชนิดเดียวกับตาราง Risk ที่เทียบ ETF คนละช่วงเวลาโดยไม่บอก

เพิ่ม `month` / `baseline_months` / `baseline_start` / `baseline_end` ให้ทุกแถว
(รูปแบบเดียวกับ `WINDOW_COLUMNS` ใน `analysis/risk.py`) ตัวเลขเดิมไม่ขยับ

### สิ่งที่ตรวจแล้วไม่พบปัญหา (รอบ 2)

* `cashflow_service` ที่เหลือ — เดือนที่ยังไม่จบถูกตัดและ**รายงาน**
  (`excluded_partial_months`), scenario หมวดซ้ำ/หมวดไม่มีจริงโยน `ValueError` (B1.2/B1.3),
  รายจ่ายติดลบถูกกันที่ `max(..., 0.0)` (B1.5), หมวดที่ฐานเป็นศูนย์ไม่ถูกทิ้ง (B1.6)
* `report_service` ที่เหลือ — `except Exception` ทุกจุดมีเหตุผลเขียนกำกับและ **re-raise**
  หลังแจ้งเตือน (cron ต้องไม่กลืนเงียบ), AI ล้มเหลวไม่ทับรายงานเดิม (M-R1),
  `has_baseline=False` ไม่ยัด 0 (M-R2), ยอดรวมกับ top_holdings มาจาก snapshot เดียวกัน (G2)

`tests/test_report_networth_pct_sign.py` (8) + `tests/test_cashflow_anomaly_window.py` (6)
พิสูจน์ด้วย mutation 3 แบบว่าแดงจริง
