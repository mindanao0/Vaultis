...
**สถานะการแก้ (อัปเดต 12 ก.ค. 2026): เฟส 1, 2 และ 3 เสร็จแล้ว** — ดูรายละเอียดท้ายไฟล์ (§ บันทึกการแก้ไข)
...
# บันทึกการแก้ไข — เฟส 3 (12 กรกฎาคม 2026)

**สถานะ: เสร็จ** — เทสต์รวม 109 ตัวผ่าน, ทุกหน้า dashboard render ผ่าน, ทดสอบ auth จากภายนอกเครื่องจริง

## นโยบายจัดสรรงบ DCA — ผู้ใช้เลือกแบบ (ข)

**สัดส่วนเป้าหมายเป็นฐาน + คะแนนเป็นตัวปรับน้ำหนัก** (`financial_model.calculate_allocation`)
ตัวคูณจากคะแนน: 0 → 0.6 เท่า | 50 → 1.0 เท่า | 100 → 1.4 เท่า แล้ว normalize

ผลกับข้อมูลจริง (งบ 5,000 บาท, โปรไฟล์ moderate):

| ETF | คะแนน | เป้าหมาย | จัดสรรจริง | ตัวคูณ | เงิน |
|---|---|---|---|---|---|
| VOO | 72.0 | 35% | 36.0% | 1.18× | 1,800 |
| SCHD | 75.0 | 25% | 26.0% | 1.20× | 1,300 |
| QQQM | 80.0 | 20% | 22.0% | 1.24× | 1,100 |
| XLV | 72.0 | 10% | 10.0% | 1.18× | 500 |
| **GLDM** | **20.0** | **10%** | **6.0%** | **0.76×** | **300** |

→ **GLDM ยังได้ซื้อทุกเดือน** (ลดจากเป้าเพราะสัญญาณอ่อน) แทนที่จะถูกตัดออกทั้งตัวแบบคะแนนล้วน
เศษจากการปัดหลักร้อยถูกแจกคืนด้วยวิธี largest-remainder → ใช้งบครบ 5,000 บาท

**พบปัญหาซ้ำรอยเดิมระหว่างทำ: มี "สัดส่วนเป้าหมาย" 2 ชุดที่ไม่ตรงกัน**
- `dashboard` / `main.py`: SCHD 20%, XLV 15%
- `rebalance_service` / `goal_service`: SCHD 25%, XLV 10%

→ แผน DCA กับแผน rebalance ดึงพอร์ตไปคนละทาง — รวมเป็นแหล่งเดียว `portfolio/targets.py`
(อ่านจาก `config.json`: `portfolio.risk_profile` + `portfolio.target_weights` ถ้าตั้งเอง)
และเพิ่มหน้าตั้งค่าให้เลือกโปรไฟล์ได้ใน dashboard

**หมายเหตุการรายงาน:** ค่า `tilt` ที่แสดงคือผลจาก**คะแนนล้วน ๆ** ไม่ปนเศษการปัดเงิน
(เดิมคำนวณจาก actual%/target% ทำให้คะแนนเท่ากันแล้วได้ 1.03× ซึ่งทำให้ผู้ใช้เข้าใจผิด)

## Auth บน backend (H1 ส่วนที่เหลือ)

`backend/security.py` — ทดสอบจากภายนอกเครื่องจริง (ยิงผ่าน IP ไม่ใช่ localhost):

| Endpoint | ไม่มี key | มี key |
|---|---|---|
| `GET /api/portfolio` (ข้อมูลส่วนตัว) | **401** | 200 |
| `POST /api/transactions/upload-slip` (เผา credit Anthropic) | **401** | — |
| `POST /api/ai/advice` (เผา credit LLM) | **401** | — |
| `GET /api/etf/prices` (อ่านอย่างเดียว) | 200 | 200 |
| `GET /health` | 200 | 200 |

- ถ้า **ไม่ตั้ง** `VAULTIS_API_KEY` → route ที่ป้องกันรับเฉพาะ localhost (deploy สาธารณะแล้วลืมตั้งคีย์จะ **fail closed** ไม่เปิดช่องให้คนอื่น)
- CORS เดิม `*` → จำกัดด้วย `VAULTIS_ALLOWED_ORIGINS`

## แก้อื่น ๆ

| ข้อ | แก้อะไร |
|---|---|
| **M2** | `optimize()` เดิมจูนและรายงานผลบนข้อมูลชุดเดียวกัน (in-sample = overfit ตัวเลขสวยเสมอ) → แบ่ง train/test 70/30 และรายงาน `test_sharpe` จากช่วงที่พารามิเตอร์ไม่เคยเห็น |
| **L4** | ถอด `streamlit` ออกจาก `analysis/macro.py`, `analysis/correlation.py`, `portfolio/dca.py` — ชั้นวิเคราะห์ไม่ควรผูกกับ UI framework และ `st.warning` ทำให้ความล้มเหลวหายเงียบใน backend |
| **L5** | `routers/analysis.py` import `analysis/` ตรง ๆ ผิดกฎของโปรเจกต์เอง → เพิ่ม `market_analysis_service.py` |
| **L6** | `datetime.utcnow()` (deprecated) → `datetime.now(UTC)`; `@app.on_event` → `lifespan`; cache ของ ai router โตไม่จำกัด → LRU 24 รายการ; `load_config()` อ่านไฟล์ทุกครั้งที่เรียก → cache ตาม mtime |

## ยังไม่ได้แก้ (ทราบและยอมรับ)

- **CI ไม่เห็นพอร์ต** — ledger เป็น CSV ที่ gitignored (บังคับ เพราะ repo เป็น public) → AI advisor รายเดือนใน GH Actions ทำงานได้แต่ไม่มีบริบทพอร์ต (และบอกตรง ๆ ว่าไม่มี ไม่ได้แกล้งทำเป็นรู้) ทางแก้จริงคือใช้ Postgres แต่ Supabase ปัจจุบันเชื่อมต่อไม่ได้
- **screener history ต้องใช้ Postgres** — ตอนนี้ `DATABASE_URL` ใช้ไม่ได้ → ประวัติ screener ไม่ถูกบันทึก (log อย่างเดียว)
- เทสต์สำหรับ forecast/vectorbt (ต้องพึ่ง network + โมเดลหนัก)

---

# บันทึกการแก้ไข — ระบบคุมค่าใช้จ่าย AI (12 กรกฎาคม 2026)

**ผู้ใช้สั่ง: "ให้ AI ทำงานเฉพาะตอนกดที่หน้าเว็บ กันเรื่องราคา API"**

## ก่อนแก้: มี 6 เส้นทางที่เรียก AI เองโดยผู้ใช้ไม่ได้สั่ง

| เส้นทาง | ความถี่ | หมายเหตุ |
|---|---|---|
| `jobs/daily_check.py` → `get_ai_advice()` | **ทุกวันทำการ** (~22 ครั้ง/เดือน) | แพงสุด — GitHub Actions |
| backend APScheduler → screener AI summary | **ทุกวัน 07:00** | ถ้า backend รันอยู่ |
| backend APScheduler → monthly report | เดือนละครั้ง | |
| `main.py` → DCA reminder | วันก่อน DCA | ยัง regex แกะตัวเลขจากข้อความ AI ด้วย |
| `main.py` → monthly advice | เดือนละครั้ง | |
| `GET /api/etf/{symbol}` | **ทุกครั้งที่ถูกเรียก** | endpoint เดียวก็เผา credit ได้ |
| `analysis/sentiment_analyzer` | ทุกสัปดาห์ | หลายครั้งต่อรอบ (ข่าวละชุด × 5 สัญลักษณ์) |

## หลังแก้: AI ปิดโดยดีฟอลต์ (`analysis/llm.py`)

`chat_text(..., user_initiated=False)` เป็นค่าเริ่มต้น → raise `LLMDisabledError`
งานอัตโนมัติทุกตัว **จับ exception นี้แล้วส่งตัวเลขจากโมเดลแทน** (ไม่ error ไม่หยุดทำงาน)

**ทดสอบจริงกับข้อมูลตลาด (ดักไว้ว่าถ้าเรียก provider = fail):**

```
1) cron รายเดือน  → เรียก AI: False | แผนจัดสรรครบ: VOO 1,800 / SCHD 1,300 /
                     QQQM 1,100 / XLV 500 / GLDM 300 บาท
2) cron รายวัน     → เรียก AI: False | ส่งสัญญาณจากโมเดลครบทั้ง 5 ตัว
3) กดปุ่มในหน้าเว็บ → เรียก AI: True
```

**สิ่งที่ยังส่งเข้า Discord ตามปกติ (ฟรี):** คะแนน, สัญญาณ, แผนจัดสรร DCA, ราคา, RSI, การเตือน alert — ซึ่งเป็นข้อมูลที่ใช้ตัดสินใจจริงอยู่แล้ว หายไปแค่ "บทความอธิบาย" ของ AI

**หน้าเว็บมี 2 ปุ่มแยกกันชัดเจน:**
- `คำนวณคะแนน + แผน DCA (ฟรี)` — ไม่เรียก AI
- `ให้ AI อธิบายด้วย (มีค่าใช้จ่าย)` — เรียก AI

PDF export มี checkbox แยก (ดีฟอลต์ปิด) และ API มี `?include_ai=true` (ดีฟอลต์ปิด)

**เปิดให้งานอัตโนมัติเรียก AI ได้:** ตั้ง `VAULTIS_LLM_AUTO=1`

**สถานะปัจจุบันของคุณ:** ไม่มี `ANTHROPIC_API_KEY` ใน `.env` → แม้กดปุ่ม AI ก็ยัง**ไม่เสียเงิน** เพราะ fallback ไป Groq (ฟรี) จะเริ่มเสียเงินก็ต่อเมื่อใส่คีย์ Anthropic

`analysis/llm.py` log จำนวนโทเคนและค่าใช้จ่ายโดยประมาณทุกครั้งที่เรียก เพื่อให้เห็นต้นทุนจริง
...
# หน้า News — ข่าวถึงตาผู้ใช้ได้โดยไม่ต้องพึ่ง LLM/ฐานข้อมูล (28 กรกฎาคม 2026)

**สถานะ: ทำแล้ว ยืนยันด้วยการ render หน้าจริง** — VOO ได้ข่าวจริง 20 + โซเชียล 10
เทสต์ทั้งชุด **340 ผ่าน** (เดิม 315 — เพิ่ม 25 ตัว)

## ปัญหา: แหล่งข่าวถูกซ่อมไปแล้ว แต่ไม่มีทางที่ข่าวจะโผล่ถึงผู้ใช้เลย

เส้นทางเดียวที่มีคือ `get_news()` → LLM ต่อบทความ → PostgreSQL → หน้าจอ
และมันขาด **3 จุด แต่ละจุดพอทำให้ผู้ใช้ไม่เห็นข่าวสักชิ้น**:

| จุดขาด | รายละเอียด |
|---|---|
| ด่านคุมค่า LLM | `run_sentiment_job()` เช็ก `auto_enabled()` ก่อน แต่ `.github/workflows/scheduler.yml` ไม่เคยตั้ง `VAULTIS_LLM_AUTO` → งานข่าวรายสัปดาห์ print "ข้าม" แล้ว return **ทุกวันจันทร์ ไม่เคยทำงานจริงสักรอบ** (ถูกต้องตามนโยบาย ไม่ใช่บั๊ก — แต่แปลว่าไม่มีข้อมูลเข้าฐาน) |
| ปลายทางตาย | `DATABASE_URL` ชี้ Supabase ที่ไม่มีอยู่แล้ว: `FATAL (ENOTFOUND) tenant/user postgres.cptzighlfdghwbfzeyiu not found` — โปรเจกต์ถูกลบ/พัก ไม่ใช่เน็ตล่ม (กระทบ screener history ด้วย) |
| key ที่ไม่ได้ตั้ง | `.env` มีแค่ `DISCORD_WEBHOOK_URL / GROQ_API_KEY / GOOGLE_API_KEY / FRED_API_KEY / DATABASE_URL` → NewsAPI และ Reddit ปิดอยู่ (Yahoo RSS + StockTwits ไม่ต้องใช้ key จึงยังทำงาน) |

ผล: กล่องบริบทในหน้า AI Advisor ขึ้น "ไม่มีข้อมูล" ตลอดกาล และ `GET /api/sentiment/{symbol}` เป็น 503 ตลอดกาล

## ทางแก้: แยกเส้นทาง "อ่านข่าว" ออกจากเส้นทาง "ให้ AI ตีความข่าว"

| จุด | ทำอะไร |
|---|---|
| `analysis/news_fetcher.py` | เพิ่ม `get_news_with_status()` + `fetch_*_status()` คู่กับ `fetch_*` เดิม (ตัวเดิมกลายเป็นเปลือกบางเรียกตัวเดียวกัน — ตรรกะการดึงมีชุดเดียว ไม่มีทางแยกกันเอง) และแยกส่วนรวม/จัดอันดับเป็น `_merge_and_rank()` ใช้ร่วมกันทั้งสองทางเข้า |
| สถานะรายแหล่ง | `ok` / `error` / `off` — **`off` (ไม่ได้ตั้ง key) ไม่ใช่ความล้มเหลว** ส่วน `error` ต้องขึ้นคำเตือน กันไม่ให้ "ดึงไม่สำเร็จ" ถูกอ่านเป็น "ไม่มีข่าว" (C1) · ครอบเคสที่เคยเงียบ: NewsAPI ตอบ 200 พร้อม `status=error` (โควตาหมด/key ถูกระงับ) และ StockTwits ตอบโครงสร้างแปลก |
| หน้า News (ใหม่) | `dashboard/app.py` — เลือก ETF, แยกหัวข้อ "ข่าว" กับ "โซเชียล" ตาม `kind`, เวลาแปลงเป็นเวลาไทย (อ่านไม่ออกแสดง `-` ไม่เดาเวลาให้ข่าว), บอกทุกครั้งว่าแหล่งไหนดึงได้กี่ชิ้น/แหล่งไหนปิดอยู่ |
| แคชไม่กลืนความล้มเหลว | `cached_news()` แคช 30 นาที แต่ถ้าแหล่งข่าวจริงพังหมดจะโยน `NewsSourcesUnavailable` — Streamlit ไม่เก็บผลของ call ที่ throw ความล้มเหลวจึงถูกลองใหม่ ไม่ค้างเป็น "ไม่มีข่าว" ครึ่งชั่วโมง (C1 แบบเดียวกับ `utils/cache.py`) |
| ค่าใช้จ่าย | **ศูนย์** — ไม่เรียก LLM ไม่แตะฐานข้อมูล ใช้ได้แม้ `DATABASE_URL` ล่มและแม้ไม่มี key สักตัว |

**ไม่ได้แก้เส้นทาง sentiment** — ยังต้องมี `VAULTIS_LLM_AUTO=1` + Postgres ที่ต่อได้เหมือนเดิม
(หน้า News ไม่ได้มาแทน sentiment แต่มาแทน "ไม่มีอะไรให้ดูเลย")

**เทสต์ที่เพิ่ม (25 ตัว)**

| ไฟล์ | คุมอะไร |
|---|---|
| `tests/test_news_status.py` (13) | error/off/ok ต้องแยกจากกัน · แหล่งเดียวพังห้ามลากแหล่งอื่นตาย · การจัดอันดับต้องเหมือน `get_news` · `fetch_*` เดิมยังคืนลิสต์เหมือนเดิม |
| `tests/test_news_page.py` (12) | ความล้มเหลวห้ามถูกแคช (เรียกซ้ำต้องลองใหม่จริง) · เวลาที่อ่านไม่ออกต้องเป็น `-` ไม่ใช่เวลาปัจจุบัน · `[ ]` ในหัวข้อห้ามทำลิงก์พัง |
...
ทั้ง 27 โมดูลหลัก import ได้หมด · ทั้ง 13 หน้า dashboard render ผ่าน (รวมหน้า News)
