...
class CategoryAnomaly(BaseModel):
    ...
    # --- หน้าต่างที่ตัวเลขข้างบนถูกวัด — ต้องขึ้นจอคู่กันเสมอ ---
    # ระวังชื่อ: ``last_month`` ข้างบนเป็น **จำนวนเงิน** (ชื่อเดิม เปลี่ยนไม่ได้เพราะ
    # ผู้เรียกผูกไว้แล้ว) ส่วน ``month`` ตรงนี้คือ **เดือน** ที่จำนวนเงินนั้นมาจาก
    #
    # ทำไมต้องมี: ``month`` คือเดือนที่จบแล้วและ **มีธุรกรรม** ล่าสุด ไม่ใช่ "เดือนที่แล้ว"
    # เสมอไป — ถ้าผู้ใช้หยุดบันทึกไป 5 เดือน API จะยังตอบ "ค่าอาหาร +76.5%" เฉย ๆ
    # แล้วหน้าจอเอาไปแสดงราวกับเป็นข่าวของเดือนนี้ (วัดจริง 2026-09-01) เป็นความผิด
    # ชนิดเดียวกับตาราง Risk ที่เทียบคนละช่วงเวลาโดยไม่บอก ⇒ ทุกตัวเลขต้องพก
    # ช่วงเวลาของตัวเองมาด้วย เหมือน ``WINDOW_COLUMNS`` ใน ``analysis/risk.py``
    month: str = ""                 # YYYY-MM ของ ``last_month``
    baseline_months: int = 0        # จำนวนเดือนที่ใช้เฉลี่ยเป็นฐาน
    baseline_start: str = ""        # YYYY-MM เดือนแรกของฐาน
    baseline_end: str = ""          # YYYY-MM เดือนสุดท้ายของฐาน
