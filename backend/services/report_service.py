...
def _holding_txt(h: dict[str, Any]) -> str:
    ...
    ``return_pct`` ที่รับมาคือ **ฐานเงินบาท** ตัวเดียวกับตัวเลขอื่นทั้งรายงานนี้
    (FIX_PLAN ข้อ 3.3 — เดิมเป็นฐานดอลลาร์ จึงอยู่คนละฐานกับยอดบาทที่วางข้าง ๆ)
    ฐานดอลลาร์อยู่ที่คีย์ ``return_pct_usd`` ถ้าวันหนึ่งรายงานต้องใช้
    """
